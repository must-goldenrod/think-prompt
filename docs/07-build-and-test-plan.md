...
> 원칙:
> - 각 마일스톤은 **1~2일** 분량.
> - 끝나면 항상 `think-prompt doctor`가 통과해야 한다.
...
**Deliverables:**
- Git repo 초기화 + `.gitignore` (node_modules, dist, `.think-prompt-test/`)
...
**Deliverables:**
- `scripts/spike-hook.sh` — 훅에서 stdin 전체를 `/tmp/think-prompt-spike/<event>-<timestamp>.json` 로 덤프
...
3. Claude Code 실행 → 다음 시나리오 수행:
   ...
   - `/compact` 실행
4. `ls /tmp/think-prompt-spike` 로 덤프된 JSON 확인.
...
- `packages/agent`:
  ...
  - pino 로깅
- `packages/cli`:
  - `think-prompt install` (settings.json 병합)
  - `think-prompt uninstall`
  - `think-prompt start` / `stop` / `status`
  - `think-prompt list` (최근 20개 출력)

**Acceptance Criteria:**
- `think-prompt install && think-prompt start` 후 Claude Code에서 아무거나 입력 → `think-prompt list`에 해당 프롬프트가 보인다.
...
# 4) 저장 확인
think-prompt list --limit 5
# 기대: 방금 친 프롬프트 첫 60자 보임

# 5) DB 직접 확인
sqlite3 ~/.think-prompt/prompts.db "SELECT id, substr(prompt_text,1,40), char_len FROM prompt_usages ORDER BY created_at DESC LIMIT 3;"
...
- 포트 점유 → `think-prompt config set agent.port 47825`.

**디버그:**
```bash
tail -f ~/.think-prompt/agent.log | pnpm pino-pretty
...
- `packages/worker`:
  ...
  - 실패 재시도 + DLQ
- `packages/cli`: `think-prompt start` 가 에이전트+워커 둘 다 기동.
...
# 확인
sqlite3 ~/.think-prompt/prompts.db "SELECT agent_type, substr(prompt_text,1,60), length(response_text) FROM subagent_invocations ORDER BY started_at DESC LIMIT 3;"

# tool rollup 확인
sqlite3 ~/.think-prompt/prompts.db "SELECT tool_name, call_count, fail_count FROM tool_use_rollups WHERE session_id=(SELECT id FROM sessions ORDER BY started_at DESC LIMIT 1);"
```

**Known Failure Modes:**
- 트랜스크립트 경로가 문서와 다름 → M0 실측 결과대로 파서 경로 조정. 여전히 실패하면 워커 로그에 "ENOENT" 확인.
- 트랜스크립트 JSON 포맷 불일치 → `TranscriptEvent.kind = 'unknown'` 과다 발생. raw 이벤트 덤프를 `/tmp/think-prompt-unknown.jsonl`에 임시 저장하고 포맷 재역공학.
- 같은 파일 반복 처리 → 멱등성 체크(§04-4.3) 확인.

**디버그:**
```bash
tail -f ~/.think-prompt/worker.log | pnpm pino-pretty
wc -l ~/.think-prompt/queue.jsonl   # 누적되는지
cat ~/.think-prompt/queue.offset    # 진행되는지
```
...
- `packages/cli`: `think-prompt show <usage_id>` 가 룰 히트 + 스코어 출력
...
- `packages/cli`: `think-prompt open` 브라우저 자동 오픈
...
**Known Failure Modes:**
- 포트 47824 점유 → `think-prompt config set dashboard.port 47825`.
...
- 세션 주입 제한 카운터(3턴 간격)
- `packages/cli`: `think-prompt coach on/off`
...
**Known Failure Modes:**
- `additionalContext`가 Claude에게 유저 메시지처럼 보여 이상한 응답 생성 → 템플릿 앞뒤에 `[Think-Prompt coaching hint]` / `[end hint]` 마커 유지.
...
- `packages/cli`: `think-prompt rewrite <usage_id>` (대시보드 버튼과 동일 경로)
...
- CLI `think-prompt rewrite --copy` 로 clipboard 복사 (`clipboardy` 사용)
...
**Test by User:**
```bash
think-prompt rewrite <usage_id> --copy
# "Copied to clipboard ✓"
...
**Deliverables:**
- `think-prompt install` 인터랙티브:
  - 기존 settings.json 백업
  - 훅 병합 (충돌 시 diff 보여주고 확인)
  - 데몬 start
  - 설치 직후 `doctor` 자동 실행
- `think-prompt upgrade` (npm 재설치 후 스키마 마이그레이션)
- `think-prompt doctor` 모든 검사 구현
- `--autostart` 옵션 (launchd/systemd-user 등록)

**Acceptance Criteria:**
- 깨끗한 환경에서 `npm i -g @think-prompt/cli && think-prompt install && claude` 한 번에 동작.
...
```bash
# 기존 설치 제거
think-prompt wipe --yes || true
npm uninstall -g @think-prompt/cli || true

# 새로 설치
npm i -g @think-prompt/cli
think-prompt install
think-prompt doctor
# 모두 ✓ 표시되어야 함
...
think-prompt doctor
# 기대: "⚠ Hook block missing — run `think-prompt install` to restore"

# 복구
think-prompt install
think-prompt doctor   # 다시 ✓
```
...
**Acceptance Criteria:**
- `npm info @think-prompt/cli` 가 0.1.0을 보여준다.
...
# (안에서)
npm i -g @think-prompt/cli
think-prompt install --non-interactive
think-prompt status
```
...
```bash
# 실시간 로그
tail -f ~/.think-prompt/agent.log ~/.think-prompt/worker.log | pnpm pino-pretty

# DB 즉석 조회
sqlite3 ~/.think-prompt/prompts.db "SELECT name FROM sqlite_master WHERE type='table';"
sqlite3 ~/.think-prompt/prompts.db "SELECT count(*) FROM prompt_usages;"

# 큐 상태
wc -l ~/.think-prompt/queue.jsonl
cat ~/.think-prompt/queue.offset
...
# 마이그레이션 상태
sqlite3 ~/.think-prompt/prompts.db "SELECT * FROM _meta;"
...
- 사용자 피드백 수집 (issue template + 간단한 `think-prompt feedback` 명령)
- 윈도우 지원
