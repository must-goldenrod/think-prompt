...
- `pro-prompt wipe --yes` 한 줄로 **모든 데이터 + 훅 완전 제거**.
...
pro-prompt uninstall       # 제거 (데이터 유지, --purge 로 완전 삭제)
pro-prompt start/stop/restart   # 데몬 제어
pro-prompt status          # 3개 데몬 상태
pro-prompt doctor          # 건강 진단 (훅 · 데몬 · DB · 로그)

pro-prompt list [--tier bad] [--rule R003] [--limit 20]
pro-prompt show <id>       # 프롬프트 상세 + 룰 히트 + 점수
pro-prompt rewrite <id> [--copy]   # LLM 리라이트 제안

pro-prompt coach on|off    # 인라인 코칭 토글
pro-prompt config get/set/list
pro-prompt reprocess [--all|--session <id>]

pro-prompt export --since 30d --out file.json
pro-prompt open            # 대시보드 브라우저 오픈
pro-prompt wipe --yes      # 완전 삭제
```
